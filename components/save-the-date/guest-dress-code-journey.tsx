...
export function GuestDressCodeJourney({
  alreadySubmitted,
  ...
  function finishIfComplete(nextCeremonies: GuestCeremonyView[]) {
    ...
    const reason = getEndReasonFromCeremonies(nextCeremonies);
    setEndReason(reason);
    setStep("end");
    notifyComplete(nextCeremonies, reason);
    return true;
  }

  function goToConfirmedEnd(nextCeremonies = ceremonyStates) {
    setEndReason("confirmed");
    setStep("end");
    notifyComplete(nextCeremonies, "confirmed");
  }
  ...
  async function downloadDressCode(targetCeremony?: GuestCeremonyView | null) {
    ...
    if (hasCeremonies && !ceremony) return;
          ...
          {step === "info" ? (
            ...
            <>
              {!hasCeremonies ? (
                <section className="invitation-panel invitation-panel--message">
                  <p className="invitation-panel__message">
                    Les célébrations se tiendront du{" "}
                    <strong>28 août au 06 septembre 2026</strong> à Kinshasa. Nous avons
